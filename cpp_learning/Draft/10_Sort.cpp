...
具体步骤：
步骤如下：
1.创建一个计数数组vecCount，用于统计每个元素出现的次数
2.遍历待排序数组vecRaw，统计每个元素出现的次数并存储在vecCount中
3.对计数数组进行进一步处理，用于统计小于等于某元素值的元素的个数
4.最后，倒序遍历待排序数组，根据元素在计数数组中的计数值，将元素放置到目标数组中的正确位置。倒序遍历的目的是为了保持相同元素的稳定性
*/
